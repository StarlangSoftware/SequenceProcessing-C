...
static int test_lstm_constructor_and_base_access(void) {
    ...
    Long_short_term_memory_model_ptr model = create_long_short_term_memory_model(parameter, 5);
    Recurrent_neural_network_model_ptr base = long_short_term_memory_model_get_base(model);
    ...
    free_long_short_term_memory_model(model);
    free_recurrent_neural_network_parameter(parameter);
    free_(optimizer);
    return success;
}

static int test_lstm_train_builds_graph_and_runs_shared_training(void) {
    Array_list_ptr hidden_layers = create_array_list();
    Array_list_ptr functions = create_array_list();
    Sigmoid_ptr sigmoid1 = create_sigmoid();
    Sigmoid_ptr sigmoid2 = create_sigmoid();
    Sigmoid_ptr sigmoid3 = create_sigmoid();
    Sigmoid_ptr sigmoid4 = create_sigmoid();
    Optimizer_ptr optimizer = create_stochastic_gradient(0.1, 0.5);
    Recurrent_neural_network_parameter_ptr parameter;
    Long_short_term_memory_model_ptr model;
    ...
    array_list_add(functions, sigmoid3);
    array_list_add(functions, sigmoid4);
    parameter = create_recurrent_neural_network_parameter(13, 2, optimizer, Random, NULL, hidden_layers, functions, 2);
    model = create_long_short_term_memory_model(parameter, 1);
    base = long_short_term_memory_model_get_base(model);
    values[0] = 0.5;
    values[1] = 1.0;
    instance = create_tensor3(values, shape, 1);
    array_list_add(train_set, instance);
    success = long_short_term_memory_model_train(model, train_set) &&
              base != NULL &&
              base->graph_initialized &&
              base->input_nodes != NULL &&
              base->switches != NULL &&
    ...
    free_sigmoid(sigmoid3);
    free_sigmoid(sigmoid4);
    free_array_list(hidden_layers, free_);
    free_array_list(functions, NULL);
    free_(optimizer);
    return success;
...
static int test_lstm_repeated_train_is_explicitly_rejected(void) {
    Array_list_ptr hidden_layers = create_array_list();
    Array_list_ptr functions = create_array_list();
    Sigmoid_ptr sigmoid1 = create_sigmoid();
    Sigmoid_ptr sigmoid2 = create_sigmoid();
    ...
    array_list_add(train_set, instance);
    success = long_short_term_memory_model_train(model, train_set) &&
              !long_short_term_memory_model_train(model, train_set) &&
              base != NULL &&
              base->graph_initialized &&
              base->input_nodes->size == 2 &&
              base->switches->size == 1 &&
...
int main(void) {
    if (!test_lstm_constructor_and_base_access()) {
        return 1;
    }
    if (!test_lstm_train_builds_graph_and_runs_shared_training()) {
        return 1;
    }
    if (!test_lstm_repeated_train_is_explicitly_rejected()) {
        return 1;
    }
    return 0;
}
